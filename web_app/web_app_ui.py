...
def a_player_data(data):
    ...
    if action == 'attack':
        collision_ID = game_engine.check_damage_area(
        ...
        if collision_ID in mainLoop.exist_monster_IDs:
            print('='*300)
            print(collision_ID)
            mainLoop.remove_monsters(collision_ID)
...
def get_player_action(data):
    ...
    if action == 'move':
        ...
        if newPos[0]:
            ...
            if type(collision_entity_id) != str:
                if collision_entity_id >= LOOT_BAG_RANGE[0] and collision_entity_id <= LOOT_BAG_RANGE[1]:
                    mainLoop.player_objects[playerID].inventory += mainLoop.loot_bag_objects[collision_entity_id].inventory
        ...
        if item_to_use in mainLoop.player_objects[playerID].inventory:
            ...
            mainLoop.player_objects[playerID].inventory.remove(item_to_use)
...
class main_loop(object):
    ...
    def monster_updater(self, current_tick, initial_setup):
        ...
        while len(self.exist_monster_IDs) != self.maxMonsters:
            x = (random.randint(MonsterSpanArea[0], MonsterSpanArea[1]) * 10)
            y = (random.randint(MonsterSpanArea[0], MonsterSpanArea[1]) * 10)
            ...
            if not(monsterID in self.exist_monster_IDs):
                ...
                for i in range(numItems): # Loop for each item to again randomly pick the item the monster will recive
                    self.monster_objects[monsterID].inventory.append(random.randint(0, len(self.ITEMS_LIST)-1))
    ...
    def bolt_updater(self, current_tick, initial_setup):
        ...
        if current_tick % boltSleep == 0:
            currentBoltsIDs = copy.deepcopy(self.exist_bolt_IDs)
            for boltID in self.exist_bolt_IDs:
                newPos = game_engine.move(
                    boltID,
                ...
                if newPos[0]:
                    ...
                    if (collision_entity_id == -1) or (collision_entity_id == self.bolt_objects[boltID].owner):
                        update_position = True
                    ...
                    if not update_position:
                        ## Also collision for edge of screen.
                        ...
                        self.bolt_objects[boltID].hasHit = True
                        currentBoltsIDs.remove(boltID)
            ...
            self.exist_bolt_IDs = copy.deepcopy(currentBoltsIDs)
